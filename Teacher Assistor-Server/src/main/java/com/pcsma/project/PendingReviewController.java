...
public class PendingReviewController {
	...
	@RequestMapping(value="/ratingTA",method = RequestMethod.GET)
			...
			if(studentRepository.findByEmail(email).getMyTask() == null)
			{
				...
				else	
				{
					...
					for(int i=0;i<length;i++)
					{
						...
						{
							if(taskRepository.findById(studentRepository.findByEmail(email).getMyTask().get(i)).getComment()==null)
							{
								finalLength=finalLength+1;
							response.put("to"+k,taskRepository.findById(studentRepository.findByEmail(email).getMyTask().get(i)).getTo());
